...
module.exports.editRow = (filePath, { data, lineNumber }) => {
    ...
    fs.readFile(filePath, 'utf8', (err, fileData) => {
        ...
        // const existingValues = lines[lineNumber].split(',');
        // for (let i = 0; i < headers.length; i++) {
        //     // const asd = JSON.parse(existingValues[i]);
        //     if (data[headers[i]] !== existingValues[i]) {
        //         return new InvalidRequestError(
        //             `Type mismatch for property "${headers[i]}": expected "${typeof existingValues[i]}", got "${typeof data[headers[i]]}"`
        //         );
        //     }
        // }
